 ...
 *
 * Piece 2a (plain concealed riichi) and Piece 2b (archetype-based)
 * hand generators. Depends on st-core.js and st-waits.js.
 */
(function (ST) {
  'use strict';
  ...
  function buildChanta(bank, ctx, junchan) {
    ...
    var pairSuits = junchan ? SUITS_NUMBERED : SUITS_NUMBERED.concat(['z']);
    var pairVals = junchan ? [1, 9] : null;
    var pair;
    if (junchan) {
      pair = tryPair(bank, pairSuits, pairVals, null);
    } else {
      var pk = [];
      for (var k in bank) {
        if (bank[k] < 2) continue;
  ...
  var ARCHETYPES = [
    { name: 'riichi-plain',    weight: 25, build: buildRiichiPlain },
    { name: 'pinfu',           weight: 18, build: buildPinfu },
    { name: 'tanyao-closed',   weight: 8,  build: function (b, c) { return buildTanyao(b, c, true); } },
    { name: 'tanyao-open',     weight: 8,  build: function (b, c) { return buildTanyao(b, c, false); } },
    { name: 'yakuhai',         weight: 10, build: buildYakuhai },
    { name: 'honitsu-closed',  weight: 3,  build: function (b, c) { return buildHonitsu(b, c, true); } },
    { name: 'honitsu-open',    weight: 3,  build: function (b, c) { return buildHonitsu(b, c, false); } },
    { name: 'chinitsu-closed', weight: 2,  build: function (b, c) { return buildChinitsu(b, c, true); } },
    { name: 'chinitsu-open',   weight: 1,  build: function (b, c) { return buildChinitsu(b, c, false); } },
    { name: 'toitoi',          weight: 3,  build: buildToitoi },
    { name: 'chiitoitsu',      weight: 3,  build: buildChiitoitsu },
    { name: 'kokushi',         weight: 1,  build: buildKokushi },
    { name: 'iipeikou',        weight: 4,  build: buildIipeikou },
    { name: 'sanshoku',        weight: 3,  build: buildSanshoku },
    { name: 'ittsuu',          weight: 2,  build: buildIttsuu },
    { name: 'chanta',          weight: 2,  build: function (b, c) { return buildChanta(b, c, false); } },
    { name: 'junchan',         weight: 1,  build: function (b, c) { return buildChanta(b, c, true); } },
    { name: 'yakuman',         weight: 1,  build: buildYakuman },
  ];
